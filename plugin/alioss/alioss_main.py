...
class alioss_main:
    ...
    def backupSite(self,name,count):
        ...
        if self.__bucket_path.find('/site/') != -1: self.__bucket_path = self.__bucket_path_bak
        ...
        #清理多余备份     
        backups = sql.table('backup').where('type=? and pid=? and filename=?',('0',pid,'alioss')).field('id,name,filename').select();
    ...
    def backupDatabase(self,name,count):
        ...
        #上传
        if self.__bucket_path.find('/database/') != -1: self.__bucket_path = self.__bucket_path_bak
        ...
        #清理多余备份     
        backups = sql.table('backup').where('type=? and pid=? and filename=?',('1',pid,'alioss')).field('id,name,filename').select();
    ...
    def backupSiteAll(self,save):
        ...
        g_path = self.__bucket_path
        for site in sites:
            self.__bucket_path = g_path
