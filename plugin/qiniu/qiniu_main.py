...
class qiniu_main:
    ...
    def __conn(self):
        ...
        if not os.path.exists(cfile): cfile = 'plugin/qiniu/config.conf';
        if not os.path.exists(cfile): return None
        fp = open(cfile,'r');
        if not fp:
    ...
    def download_file(self,filename):
        self.__conn();
        try:
            base_url = '%s/%s' % (self.__bucket_domain, filename)
            private_url = self.__qiniu.private_download_url(base_url, expires=3600)
            return private_url.strip().replace('http//','')
        except:
            print(self.__error_msg)
            return False
    ...
    def backupSite(self,name,count):
        ...
        #清理多余备份     
        backups = sql.table('backup').where('type=? and pid=? and filename=?',('0',pid,'qiniu')).field('id,name,filename').select();
    ...
    def backupDatabase(self,name,count):
        ...
        #清理多余备份     
        backups = sql.table('backup').where('type=? and pid=? and filename=?',('1',pid,'qiniu')).field('id,name,filename').select();
