...
class load_leveling_main:
    ...
    def get_loadleveling_list(self, get):
        if public.ExecShell('nginx -V 2>&1|grep nginx-sticky-module')[
    ...
    def cerate_loadleveling(self, get):
        if get.session_type=='ip_hash':
            rec=[]
            for i in json.loads(get.upnodes):
                if i['state']==2:
                    return public.returnMsg(False, 'IP回话跟跟随中不能存在备份节点')
        upName = get.upname;
        ...
        # get.sub2 = '';
        # get.type = '1';
        get.sitename = mainDomain
        get.proxyname = upName
        get.proxysite = 'http://' + upName
        get.todomain = '$host'
        get.proxydir = '/'
        get.type = 1
        get.cache = 0
        ...
        if result['status']:
            ...
            public.WriteLog('负载均衡', '添加负载均衡[' + upName + ']' + '成功')
        else:
            public.WriteLog('负载均衡', '添加负载均衡[' + upName + ']' + '失败')
...
location ~ \.well-known{
    ...
    def add_node(self, get):
        if not get.path:return public.returnMsg(False, '不能为空');
        if str(get.path[0]) != '/':return public.returnMsg(False, '例如/aa.txt');
        upName = get.upname
        ...
        for i in range(len(data)):
            ...
            if data[i]['name']==upName:
                if str(data[i]['session_type'])=='ip_hash':
                    if int(get.state) ==2:
                        return public.returnMsg(False, 'IP回话跟跟随中不能存在备份节点')
    ...
    def modify_node(self, get):
        ...
        for i in range(len(data)):
            ...
            for n in range(len(data[i]['nodes'])):
                ...
                data[i]['nodes'][n]['path']= str(get.path)
    ...
    def get_node_list(self, get):
        ...
        for i in range(len(levelingInfo['nodes'])):
            if levelingInfo['nodes'][i]['port']==443:
                url = 'https://' + levelingInfo['nodes'][i]['server'].strip() + str(levelingInfo['nodes'][i]['path'])
            else:
                url = 'http://' + levelingInfo['nodes'][i]['server'].strip() + ':' + str(levelingInfo['nodes'][i]['port']) + \
                      str(levelingInfo['nodes'][i]['path'])
...
%s
    ...
    def _check_node(self):
        ...
        for i in range(len(data)):
            for n in range(len(data[i]['nodes'])):
                ...
                if not status:
                    public.WriteLog('负载均衡', '负载 %s  域名中  %s  节点检测失败 ' % (str(data[i]['site']),node['server'].strip()))
                if status and not nerror:
                    public.WriteLog('负载均衡', '负载   %s    域名中节点 %s  已经恢复 ' % (str(data[i]['site']),node['server'].strip()))
        self.__write_config(data)
    ...
    def check_url(self,get):
        ...
        if not ret:return public.returnMsg(False, '%s访问失败'%url)
        return public.returnMsg(True, '添加成功')

    # 删除日志
    def del_log(self,get):
        ret=public.M('logs').where('type=?', (u'负载均衡',)).delete()
