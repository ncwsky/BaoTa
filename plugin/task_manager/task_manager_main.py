...
class task_manager_main:
    ...
    def get_network_list(self,get):
        ...
        for netstat in netstats:
            ...
            tmp['process']  = p.name()
            tmp['pid']      = netstat.pid
            if tmp['process'] == 'gunicorn': continue
    ...
    def is_panel_process(self,pid):
        if not self.panel_pid:
            self.panel_pid = int(public.ExecShell("ps aux | grep 'runserver:app'|grep -v grep|head -n1|awk '{print $2}'")[0])
    ...
    def get_run_ps(self,name):
        runPs = {'netconsole':'网络控制台日志','network':'网络服务','jexec':'JAVA','tomcat8':'Apache Tomcat','tomcat7':'Apache Tomcat','mariadb':'Mariadb',
                 ...
                 'php-fpm-54':'PHP-5.4','php-fpm-55':'PHP-5.5','php-fpm-56':'PHP-5.6','php-fpm-70':'PHP-7.0','php-fpm-71':'PHP-7.1','gunicorn':'宝塔面板Web进程',
    ...
    def get_process_ps(self,name,pid):
        processPs = {'mysqld':'MySQL服务','php-fpm':'PHP子进程','php-cgi':'PHP-CGI进程',
                     ...
                     'sftp-server':'SFTP服务','mysqld_safe':'MySQL服务','firewalld':'防火墙服务','gunicorn':'宝塔面板HTTP服务',
        ...
        if name == 'python':
            if self.is_panel_process(pid):
                if self.task_pid == pid: return '宝塔面板任务队列进程'
                return '宝塔面板';
        return name;
